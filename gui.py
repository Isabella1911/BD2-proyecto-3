...
class PuzzleSolverGUI:
    ...
    def _build_ui(self):
        ...
        self.canvas = tk.Canvas(self.root, bg="white",
                                width=600, height=380, highlightthickness=1,
        ...
        self.btn_prev = tk.Button(
            ctrl, text="◀  Anterior", width=13,
            command=self._prev_step, state=tk.DISABLED,
            font=("Arial", 11), bg="#DDDDDD"
        )
        self.btn_prev.pack(side=tk.LEFT, padx=6)

        self.btn_start = tk.Button(
            ctrl, text="Elegir pieza inicial", width=18,
            command=self._choose_start,
            font=("Arial", 11), bg="#FFD700"
        )
        self.btn_start.pack(side=tk.LEFT, padx=6)

        self.btn_next = tk.Button(
            ctrl, text="Siguiente  ▶", width=13,
    ...
    def _on_puzzle_selected(self, _event=None):
    ...
    def _resize_canvas(self):
        ...
        self.canvas.config(width=max(w, 400), height=max(h, 300))

    def _cell_center(self, x, y):
        return PAD + x * CELL_W + CELL_W // 2, PAD + y * CELL_H + CELL_H // 2

    def _draw_piece(self, pid, color):
        p  = self.pieces[pid]
        cx, cy = self._cell_center(p["x"], p["y"])
        hw, hh = CELL_W // 2 - 4, CELL_H // 2 - 4
        tag = f"piece_{pid}"
        self.canvas.delete(tag)
        self.canvas.create_rectangle(
            cx - hw, cy - hh, cx + hw, cy + hh,
            fill=color, outline="#444444", width=2, tags=tag
        )
    ...
    def _draw_arrow(self, from_id, to_id):
        ...
        self.canvas.create_line(
            fx, fy, tx, ty,
            arrow=tk.LAST, fill="#D04000", width=3,
            arrowshape=(14, 18, 6), tags="arrow"
        )

    def _redraw_all(self):
        self.canvas.delete("all")
        for pid, p in self.pieces.items():
            base = C_UNPLACED if p["available"] else C_MISSING
            self._draw_piece(pid, base)
    ...
    # ── BFS (mirrors proyecto3.py logic) ──────────────────────────────────
    def _compute_steps(self):
        ...
        self.step_label.config(text=f"Pieza inicial: {self.start_id}   —   Paso 0 de {total}")
        self.desc_label.config(text=f"Coloca la pieza {self.start_id} como punto de partida.")
    ...
    def _next_step(self):
        ...
        # repaint: all placed pieces back to their settled colour,
        ...
        if avail:
            self.desc_label.config(
                text=f"Coloca la pieza  {to_id}  al  {DIR_ES[direc]}  de la pieza  {from_id}."
            )
        else:
            self.desc_label.config(
    ...
    def _prev_step(self):
        ...
        if self.current_step >= 0:
            ...
            self.desc_label.config(
                text=(
                    f"Coloca la pieza  {to_id}  al  {DIR_ES[direc]}  de la pieza  {from_id}."
                    if avail else
